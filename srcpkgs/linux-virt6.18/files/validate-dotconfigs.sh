...
for arch in x86_64 arm64; do
	...
	for symbol in \
		EFI EFI_STUB EFI_PARTITION PCI PCI_MSI \
		...
		DRM_SIMPLEDRM DRM_VIRTIO_GPU DRM_VIRTIO_GPU_KMS DRM_QXL EXT4_FS XFS_FS \
		BTRFS_FS EFIVAR_FS; do
	...
	if [ "$arch" = arm64 ]; then
		for symbol in ARM_GIC_V3 ARM_PSCI_CPUIDLE SERIAL_AMBA_PL011 \
